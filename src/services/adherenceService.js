...
export const processMissedAdherenceRecords = async () => {
  ...
  // Consider any scheduled dose from more than 3 hours ago as missed if still pending
  const cutoffTime = new Date(now.getTime() - 3 * 60 * 60 * 1000);
  
  try {
    // Find all pending adherence records that should have been taken by now
    const pendingRecords = await Adherence.find({
      status: 'pending',
      scheduledDate: { $lt: now.setHours(0, 0, 0, 0) }, // From previous days
    });
    
    // Also find records from today that are past the cutoff time
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    const todayPendingRecords = await Adherence.find({
      status: 'pending',
      scheduledDate: today,
    }).populate('medication');
    
    const missedTodayRecords = todayPendingRecords.filter(record => {
      const [hours, minutes] = record.scheduledTime.split(':').map(Number);
      const scheduledDateTime = new Date(record.scheduledDate);
      scheduledDateTime.setHours(hours, minutes, 0, 0);
      
      return scheduledDateTime < cutoffTime;
    });
    
    const allMissedRecords = [...pendingRecords, ...missedTodayRecords];
    
    // Update status to missed
    const updatePromises = allMissedRecords.map(record => {
    ...
    logger.info(`Processed ${allMissedRecords.length} missed adherence records`);
    return allMissedRecords.length;
  } catch (error) {
