...
export const mockReminders: Reminder[] = [
  ...
  },
  {
    ...
    message: "Time to take your Vitamin D3 (2000 IU)",
    retry_count: 2,
    last_retry: "2024-01-16T09:15:00Z",
    created_at: "2024-01-15T00:00:00Z",
...
// Generate more reminders for different dates
export const generateMoreReminders = () => {
  const additionalReminders: Reminder[] = [];
  const today = new Date();
  ...
  for (let i = 0; i < 7; i++) {
    ...
    mockMedications.forEach((med, medIndex) => {
      med.scheduled_times.forEach((time, timeIndex) => {
        additionalReminders.push({
          id: `rem-gen-${i}-${medIndex}-${timeIndex}`,
          user_id: "user-1",
          medication_id: med.id,
          scheduled_time: time,
          scheduled_date: date.toISOString().split("T")[0],
          status: i === 0 ? "pending" : Math.random() > 0.7 ? "failed" : "sent",
          channels: {
            email: {
              enabled: true,
              sent: i !== 0 && Math.random() > 0.3,
            },
            sms: {
              enabled: Math.random() > 0.5,
              sent: i !== 0 && Math.random() > 0.5,
            },
          },
          message: `Time to take your ${med.name} (${med.dosage.amount}${med.dosage.unit})`,
          retry_count: Math.random() > 0.8 ? Math.floor(Math.random() * 3) : 0,
          last_retry:
            Math.random() > 0.8
              ? new Date(date.getTime() + Math.random() * 3600000).toISOString()
              : undefined,
...
export const allMockReminders = [...mockReminders, ...generateMoreReminders()];
